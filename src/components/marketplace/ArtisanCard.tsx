  ...
  return (
    <Link to={`/craftsman/${artisan.id}`} className="artisan-card group block">
      <div className="relative aspect-[4/5] overflow-hidden bg-sand">
        {image ? (
          <img src={image} alt={`Work by ${name}`} className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.04]" loading="lazy" decoding="async" />
        ) : (
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-charcoal/80 via-charcoal/25 to-transparent p-4 pt-16">
          <p className="font-display text-xl leading-tight text-cream">{name}</p>
